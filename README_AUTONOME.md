...
Le projet inclut désormais un backend Express avec Drizzle ORM/MySQL. Copiez `.env.example` vers `.env`, renseignez `DATABASE_URL` et `JWT_SECRET`, puis exécutez `pnpm run db:push` et `pnpm run db:seed`. Le back-office est accessible sur `/admin`. En l’absence de `DATABASE_URL`, les routes publiques utilisent un contenu de secours et le site vitrine reste disponible.

Commandes utiles : `pnpm run db:generate`, `pnpm run db:push`, `pnpm run db:seed`, `pnpm run check`, `pnpm run build`.
