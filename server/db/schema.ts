...
import { index, int, json, mysqlEnum, mysqlTable, serial, text, timestamp, varchar } from "drizzle-orm/mysql-core";
...
export const adminLogs = mysqlTable("admin_logs", { id: serial("id").primaryKey(), adminId: int("admin_id").references(() => adminUsers.id, { onDelete: "set null" }), action: varchar("action", { length: 100 }).notNull(), targetEntity: varchar("target_entity", { length: 100 }), targetId: int("target_id"), details: text("details"), ipAddress: varchar("ip_address", { length: 45 }), createdAt: timestamp("created_at").defaultNow().notNull() });
